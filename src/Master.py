...
job_sockets - dict with key value as socket id and value as job sockets
...
def RR_ALGO():
    ...
    sorted(localData, key=itemgetter("worker_id"))
...
def LL_ALGO():
    ...
    config_race_lock.release()
    # Sorts the list in the Ascending order of worker ID
    sorted(localData, key=itemgetter("worker_id"), reverse=True)
    while 1:
...
def master2worker_send(job):
    for i in job:
        ...
        pos = ports.index(portNumber)  # pos contains the port to be connected
        w = pos
        if isinstance(i, dict):
            tid = i["task_id"]
            tid_s = tid.split('_')
            stroc = str(pos + 2)
            Client2Socket, _ = job_sockets["soc" + str(stroc)].accept()
...
def request2master_jobs():
    while True:
        Client2Socket, _ = job_sockets["soc0"].accept()
        ...
        red = Message['reduce_tasks']

        req_race_lock.acquire()
        requests_data[job_id] = []
        requests_data[job_id].extend((len(map), len(red), red))
...
def worker2master_listen():  
    global ConfigData
    while True:
        Client2Socket, _ = job_sockets["soc1"].accept()
        ...
        if(len(Message) > 0):
            ...
            print("Received update from worker: ", Message, " from the port", linkport)
            for i in ConfigData:
    ...
    while 1: 
        ...
        for i in list(keys):
            if requests_data[i][0] == 0:
                iteratedKeys.add(i)
                master2worker_send(requests_data[i][2])
...
def masterWorker_connect(i, j):  
    while 1:  
        try:  
            ...
            print('Ready to Establish A Connection ', time_2, " on Port with Index ", str(j + 2))
            Client2Socket, _ = job_sockets["soc" + str(j + 2)].accept()
...
def master2worker_init():
    init_list = []
    for i in range(3):
        init_list.append(th.Thread(target=masterWorker_connect, args=(config_data_original[i], i)))  
...
def main():
    ...
    for i in range(5):
        job_sockets["soc" + str(i)] = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

    job_sockets["soc0"].bind(('', 5000))    # Ready for incoming jobs
 
    job_sockets["soc0"].listen(5)

    job_sockets["soc1"].bind(('', 5001))    # ready to worker updates
 
    job_sockets["soc1"].listen(5)

    for i in range(2, 5):
        
        job_sockets["soc" + str(i)].bind((socket.gethostname(), ports[i - 2]))
        for j in config_data_original:
            if j['port'] == ports[i - 2]:
               
                number = int(j['slots'])  # assigning the connections
                job_sockets["soc" + str(i)].listen(number)
                break
